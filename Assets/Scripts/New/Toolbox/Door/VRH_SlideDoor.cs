...
public class VRH_SlideDoor : VRH_DoorBase
{
    ...
    [HideInInspector] public float OpenDistance = 0.0f;
    [HideInInspector] public Vector3 OpenPosition = new Vector3();
    ...
    private void Update()
    {
        ...
        if (isMove)
        {
            if (time < OpenDistance)
            {
                time += Time.deltaTime * OpenSpeed;
                if (isOpened)
                {
                    transform.position = Vector3.MoveTowards(transform.position, startTransform.position, Time.deltaTime * OpenSpeed);
                }
                else
                {
                    transform.position = Vector3.MoveTowards(transform.position, OpenPosition, Time.deltaTime * OpenSpeed);
                }
            }
            else
            {
